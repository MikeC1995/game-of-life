...
int _openinpgm(const char fname[], int width, int height)
{
	...
	//Strip off header: (See for PGM format: http://netpbm.sourceforge.net/doc/pgm.html)
	//Remember fgets reads to whitespace.
    fgets( str, 64, _INFP ); //Version: P5
    fgets( str, 64, _INFP ); //width and height
    sscanf( str, "%d%d", &inwidth, &inheight );
    if( inwidth != width || inheight != height )
    {
    	printf( "WARNING: Input image size(%dx%d) does not = %dx%d or trouble reading header\n", inwidth, inheight, width, height );
    	//return -1;
    }
...
/////////////////////////////////////////////////////////////////////////////////////////////
//Line-wise pgm out:
int _openoutpgm(const char fname[], int width, int height)
{
    char hdr[ 64 ];
